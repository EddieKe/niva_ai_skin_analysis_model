...
def makeup_recommendation(skin_tone, skin_type):
    ...
    dataframes_to_concat.append(makeup[(makeup['skin tone'] == skin_tone) & (makeup['skin type'] == skin_type) & (makeup['label'] == 'foundation')].head(2))
    dataframes_to_concat.append(makeup[(makeup['skin tone'] == skin_tone) & (makeup['skin type'] == skin_type) & (makeup['label'] == 'concealer')].head(2))
    dataframes_to_concat.append(makeup[(makeup['skin tone'] == skin_tone) & (makeup['skin type'] == skin_type) & (makeup['label'] == 'primer')].head(2))

    # Use pd.concat() to combine all the dataframes in the list
    dff = pd.concat(dataframes_to_concat, ignore_index=True)
    ...
    return result
#Following removal of forms we will tweak the algrothm to recommend products based of the ai skin analysis results
